    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Fornecedor</li>
          <li>✓ Controle completo de Medicamento</li>
          <li>✓ Controle completo de Venda</li>
          <li>✓ Controle completo de Item Venda</li>
        </ul>
      </div>
...
function PainelFornecedor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cnpj", "email", "telefone", "endereco", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/fornecedors", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/fornecedors/" + editId : apiBaseUrl + "/api/fornecedors";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cnpj", "email", "telefone", "endereco"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/fornecedors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Fornecedor")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Fornecedor</h3>
...
function PainelMedicamento({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "principioAtivo", "descricao", "categoria", "preco", "estoque", "estoqueMinimo", "fornecedorId", "prescricaoObrigatoria", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/medicamentos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/fornecedors", { headers: authHeaders(token) }).then(r => r.json()).then(setFornecedorList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/medicamentos/" + editId : apiBaseUrl + "/api/medicamentos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "principioAtivo", "descricao", "categoria", "preco", "estoque", "estoqueMinimo", "fornecedorId", "prescricaoObrigatoria"].forEach(k => { f[k] = item[k] ?? ""; });
    ["fornecedor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/medicamentos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">M</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/medicamento" + item.id + "/320/200"} alt="Medicamento" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["nome"] ?? "Medicamento")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>principioAtivo</b><span>{item["principioAtivo"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>fornecedorId</b><span>{item["fornecedorId"] ?? "—"}</span></div>
            <div className="item-field"><b>prescricaoObrigatoria</b><span>{item["prescricaoObrigatoria"] != null ? (item["prescricaoObrigatoria"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>fornecedor</b><span>{item.fornecedorId ? ((fornecedorList.find(o => o.id === item.fornecedorId) || {}).nome ?? ("#" + item.fornecedorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Medicamento</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      <label className="field-label">principioAtivo</label>
      <input type="text" value={form["principioAtivo"] ?? ""} onChange={e => setForm({...form, principioAtivo: e.target.value})} />
      ...
      <label className="field-label">fornecedorId</label>
      <input type="number" value={form["fornecedorId"] ?? ""} onChange={e => setForm({...form, fornecedorId: e.target.value})} />
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["prescricaoObrigatoria"] === true || form["prescricaoObrigatoria"] === "true"} onChange={e => setForm({...form, prescricaoObrigatoria: e.target.checked})} />
        prescricaoObrigatoria
      </label>
      ...
      <select value={form["fornecedor"] ?? ""} onChange={e => setForm({...form, fornecedor: e.target.value})}>
        ...
        {(fornecedorList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelVenda({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["dataVenda", "valorTotal", "formaPagamento", "desconto", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/vendas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/vendas/" + editId : apiBaseUrl + "/api/vendas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["dataVenda", "valorTotal", "formaPagamento", "desconto", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["fornecedor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/vendas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
    ...
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["formaPagamento"] ?? "Venda")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/vendas/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">V</div>
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/venda" + item.id + "/320/200"} alt="Venda" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🧾</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["formaPagamento"] ?? "Venda")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>dataVenda</b><span>{item["dataVenda"] ? new Date(item["dataVenda"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            ...
            <div className="item-field"><b>desconto</b><span>{item["desconto"] != null ? Number(item["desconto"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>fornecedor</b><span>{item.fornecedorId ? ((fornecedorList.find(o => o.id === item.fornecedorId) || {}).nome ?? ("#" + item.fornecedorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Venda</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">dataVenda</label>
      <input type="datetime-local" value={form["dataVenda"] ?? ""} onChange={e => setForm({...form, dataVenda: e.target.value})} />
...
function PainelItemVenda({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [medicamentoList, setMedicamentoList] = React.useState([]);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["vendaId", "medicamentoId", "quantidade", "valorUnitario", "valorTotal", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/itemvendas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/medicamentos", { headers: authHeaders(token) }).then(r => r.json()).then(setMedicamentoList).catch(() => {});
    fetch(apiBaseUrl + "/api/vendas", { headers: authHeaders(token) }).then(r => r.json()).then(setVendaList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/itemvendas/" + editId : apiBaseUrl + "/api/itemvendas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["vendaId", "medicamentoId", "quantidade", "valorUnitario", "valorTotal"].forEach(k => { f[k] = item[k] ?? ""; });
    ["venda", "medicamento"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/itemvendas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{"ItemVenda"}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>vendaId</b><span>{item["vendaId"] ?? "—"}</span></div>
            <div className="item-field"><b>medicamentoId</b><span>{item["medicamentoId"] ?? "—"}</span></div>
            <div className="item-field"><b>quantidade</b><span>{item["quantidade"] ?? "—"}</span></div>
            <div className="item-field"><b>valorUnitario</b><span>{item["valorUnitario"] != null ? Number(item["valorUnitario"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>valorTotal</b><span>{item["valorTotal"] != null ? Number(item["valorTotal"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>venda</b><span>{item.vendaId ? ((vendaList.find(o => o.id === item.vendaId) || {}).formaPagamento ?? ("#" + item.vendaId)) : "—"}</span></div>
            <div className="item-field"><b>medicamento</b><span>{item.medicamentoId ? ((medicamentoList.find(o => o.id === item.medicamentoId) || {}).nome ?? ("#" + item.medicamentoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} ItemVenda</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">vendaId</label>
      <input type="number" value={form["vendaId"] ?? ""} onChange={e => setForm({...form, vendaId: e.target.value})} />
      <label className="field-label">medicamentoId</label>
      <input type="number" value={form["medicamentoId"] ?? ""} onChange={e => setForm({...form, medicamentoId: e.target.value})} />
      <label className="field-label">quantidade</label>
      <input type="number" value={form["quantidade"] ?? ""} onChange={e => setForm({...form, quantidade: e.target.value})} />
      <label className="field-label">valorUnitario</label>
      <input type="number" step="0.01" value={form["valorUnitario"] ?? ""} onChange={e => setForm({...form, valorUnitario: e.target.value})} />
      <label className="field-label">valorTotal</label>
      <input type="number" step="0.01" value={form["valorTotal"] ?? ""} onChange={e => setForm({...form, valorTotal: e.target.value})} />
      <label className="field-label">venda</label>
      <select value={form["venda"] ?? ""} onChange={e => setForm({...form, venda: e.target.value})}>
        <option value="">Selecione...</option>
        {(vendaList || []).map(o => (<option key={o.id} value={o.id}>{o.formaPagamento ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">medicamento</label>
      <select value={form["medicamento"] ?? ""} onChange={e => setForm({...form, medicamento: e.target.value})}>
        <option value="">Selecione...</option>
        {(medicamentoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Fornecedor");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Farmacia<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Fornecedor" ? " active" : "")} onClick={() => setAba("Fornecedor")}><span className="nav-ico">🗂️</span>Fornecedor</button>
        <button className={"nav-btn" + (aba === "Medicamento" ? " active" : "")} onClick={() => setAba("Medicamento")}><span className="nav-ico">🗂️</span>Medicamento</button>
        <button className={"nav-btn" + (aba === "Venda" ? " active" : "")} onClick={() => setAba("Venda")}><span className="nav-ico">🧾</span>Venda</button>
        <button className={"nav-btn" + (aba === "ItemVenda" ? " active" : "")} onClick={() => setAba("ItemVenda")}><span className="nav-ico">📦</span>ItemVenda</button>
      ...
      <main className="main">
        ...
        {aba === "Fornecedor" && <PainelFornecedor token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Medicamento" && <PainelMedicamento token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Venda" && <PainelVenda token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "ItemVenda" && <PainelItemVenda token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
